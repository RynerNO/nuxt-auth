<template lang="pug">
  v-form(v-model="loginForm.valid")
    ...
    v-card-text.elevation-12.d-flex.flex-column
      ...
      v-btn.align-self-end(:disabled="!loginForm.valid" color="primary" @click="login") Login
</template>
...
export default {
  ...
  methods: {
    login() {
      this.$store.dispatch('auth/POST_LOGIN', this.loginForm.fields).then(() => {
          this.$router.push('/')
      })
    }
  }
